...
// This file has divisions by zero to tests isnan
...
#	ifdef NDEBUG
	...
	int i, m, n;
	static unsigned tests[] = {0,32, 1,0, 2,1, 3,0, 4,2, 5,0, 6,1,  7,0,
	...
	for (i = 0; i < n; i += 2) {
		if (ntz1(tests[i]) != tests[i+1]) error(tests[i], ntz1(tests[i]));}
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz2(tests[i]) != tests[i+1]) error(tests[i], ntz2(tests[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz3(tests[i]) != tests[i+1]) error(tests[i], ntz3(tests[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz4(tests[i]) != tests[i+1]) error(tests[i], ntz4(tests[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz4a(tests[i]) != tests[i+1]) error(tests[i], ntz4a(tests[i]));}
	TimestampEnd = std::clock();
	...
	for(i = 0; i < n; i += 2)
	{
		...
		if(m > 8)
			m = 8;
		if(ntz5(static_cast<char>(tests[i])) != m)
			error(tests[i], ntz5(static_cast<char>(tests[i])));
	}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz6(tests[i]) != tests[i+1]) error(tests[i], ntz6(tests[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz6a(tests[i]) != tests[i+1]) error(tests[i], ntz6a(tests[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz7(tests[i]) != tests[i+1]) error(tests[i], ntz7(tests[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz7_christophe(tests[i]) != tests[i+1]) error(tests[i], ntz7(tests[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz8(tests[i]) != tests[i+1]) error(tests[i], ntz8(tests[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz8a(tests[i]) != tests[i+1]) error(tests[i], ntz8a(tests[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz9(tests[i]) != tests[i+1]) error(tests[i], ntz9(tests[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz10(tests[i]) != tests[i+1]) error(tests[i], ntz10(tests[i]));}
	TimestampEnd = std::clock();

	printf("ntz10: %d clocks\n", static_cast<int>(TimestampEnd - TimestampBeg));

	if (errors == 0)
		printf("Passed all %d cases.\n", static_cast<int>(sizeof(tests)/8));
